...
SMOKE_APP ?= gui/phase2_smoke.uya
...
build:
	...
	$(UYA) build $(SMOKE_APP) $(UYA_OPT) -o $(ABS_BUILD_DIR)/phase2_smoke
...
build-arm:
	...
	TARGET_ARCH=arm TARGET_OS=none $(UYA) build --c99 $(SMOKE_APP) $(UYA_OPT) -o $(ABS_BUILD_DIR)/c99/phase2_smoke_arm.c

build-riscv:
	@mkdir -p $(ABS_BUILD_DIR)/microapp
	$(UYA) build --app microapp --microapp-profile rv32_baremetal_softvm $(SMOKE_APP) $(UYA_OPT) -o $(ABS_BUILD_DIR)/microapp/phase2_smoke_rv32.pobj

build-esp32:
	@mkdir -p $(ABS_BUILD_DIR)/microapp
	$(UYA) build --app microapp --microapp-profile xtensa_baremetal_softvm $(SMOKE_APP) $(UYA_OPT) -o $(ABS_BUILD_DIR)/microapp/phase2_smoke_xtensa.pobj
